...
export default {
  methods: {
    ...
    async showModifyWindow(event: {
      ...
      const [_maleSingle, _femaleSingle, _malePlural, _femalePlural] =
    ...
    async showAddWindow() {
      ...
      if (formValues) {
        ...
        if (!apiResponse.isError && apiResponse.code === 201) {
          const newItems = GuiModify.afterBinyanInsert(
            this.items,
            apiResponse.data
          );
          this.items = newItems;
    ...
    async showRestoreWindow() {
      const Toast = getToast(this.$swal);
      const confirmation = await showConfirmation(this.$swal, "биньяны", false);
      if (confirmation.isConfirmed) {
...
<template>
  ...
  <div>
    <input v-on:change="searchByWord" type="text" placeholder="Поиск биньяна" />
    <input
