...
export default {
  methods: {
    searchByWord(event: {
      ...
      setTimeout(async () => {
        ...
        let items;
        if (!q) {
          items = await Api.doGetRequest("/war-words/getAllItems");
        } else {
          items = await Api.doGetRequest(`/war-words/search?q=${q}&admin=true`);
        }
    ...
    searchByPage(event: {
      ...
      setTimeout(async () => {
        ...
        const items = await Api.doGetRequest(`/war-words/getAllItems?page=${page}`);
        if (!items || items.length === 0) {
          Toast.fire({
            icon: "error",
    ...
    async showAddWindow() {
      ...
      if (formValues) {
        const [ru, translit, he] = formValues;
        const apiResponse = await Api.doPostRequest("/war-words/addNew", {
          ru,
          translit,
          he,
        });
    ...
    async showRestoreWindow() {
      ...
      if (confirmation.isConfirmed) {
        const apiResponse = await Api.doGetRequest("/war-words/restore");
    ...
    sortAsc(event: {
      ...
      setTimeout(async () => {
        ...
        const items = await Api.doGetRequest(
          `/war-words/getAllItems?sort=1&page=${currentPage}`
        );
        this.items = items;
        this.loading = false;
      }, DELAY_API_REQUEST_MS);
  ...
  async created() {
    const items = await Api.doGetRequest("/war-words/getAllItems");
    this.items = items;
    this.loading = false;
  },
};
