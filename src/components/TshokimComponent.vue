...
export default {
  methods: {
    ...
    async showAddWindow() {
      ...
      if (formValues) {
        ...
        const apiResponse = await Api.doPostRequest("/tshokim/addNew", {
          ru,
          translit,
          he,
        });
    ...
    async showRestoreWindow() {
      const Toast = getToast(this.$swal);
      const confirmation = await showConfirmation(this.$swal, "шутки", false);
      if (confirmation.isConfirmed) {
        const apiResponse = await Api.doGetRequest("/tshokim/restore");
        if (apiResponse && apiResponse.restored) {
          Toast.fire({
            icon: "success",
            title: "Все шутки восстановлены из резервной копии (обновите страницу)",
          });
        } else {
          console.error(apiResponse);
          Toast.fire({
    ...
    sortAsc(event: {
      ...
      setTimeout(async () => {
        ...
        const items = await Api.doGetRequest(
          `/tshokim/getAllItems?sort=1&page=${currentPage}`
        );
        this.items = items;
        this.loading = false;
      }, DELAY_API_REQUEST_MS);
  ...
  async created() {
    const items = await Api.doGetRequest("/tshokim/getAllItems");
    this.items = items;
    this.loading = false;
  },
};
